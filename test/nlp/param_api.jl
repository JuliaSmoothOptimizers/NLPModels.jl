@testset "Parametric API" begin
  ...
  n = get_nvar(nlp.meta)
  m = get_ncon(nlp.meta)
  np = get_nparam(nlp.meta)
  p1, p2 = nlp.ps

  @test np == 2
  @test get_nnzjp(nlp.meta)    == 1
  @test get_nnzhp(nlp.meta)    == 3
  @test get_nnzgp(nlp.meta)    == 2
  @test get_nnzjplcon(nlp.meta) == 1
  @test get_nnzjpucon(nlp.meta) == 1
  @test get_nnzjplvar(nlp.meta) == 2
  @test get_nnzjpuvar(nlp.meta) == 2
  ...
  @testset "jac_param" begin
    ...
    vals2 = zeros(get_nnzjp(nlp.meta))
  ...
  @testset "hess_param" begin
    ...
    vals2 = zeros(get_nnzhp(nlp.meta))
  ...
  @testset "lcon_jac" begin
    ...
    @test vals ≈ [-1.0]
    vals2 = zeros(get_nnzjplcon(nlp.meta))
  ...
  @testset "ucon_jac" begin
    ...
    @test vals ≈ [1.0]
    vals2 = zeros(get_nnzjpucon(nlp.meta))
  ...
  @testset "lvar_jac" begin
    ...
    @test vals ≈ [-1.0, -1.0]
    vals2 = zeros(get_nnzjplvar(nlp.meta))
  ...
  @testset "uvar_jac" begin
    ...
    @test vals ≈ [1.0, 1.0]
    vals2 = zeros(get_nnzjpuvar(nlp.meta))
  ...
  @testset "non-parametric behavior" begin
    ...
    @test get_nparam(plain.meta)    == 0
    @test get_nnzjp(plain.meta)     == 0
    @test get_nnzhp(plain.meta)     == 0
    @test get_nnzgp(plain.meta)     == 0
    @test get_nnzjplcon(plain.meta) == 0
    @test get_nnzjpucon(plain.meta) == 0
    @test get_nnzjplvar(plain.meta) == 0
    @test get_nnzjpuvar(plain.meta) == 0
    @test !get_grad_param_available(plain.meta)
    @test !get_jac_param_available(plain.meta)
    @test !get_hess_param_available(plain.meta)
  end
end
