...
function NLPModels.jth_con(nlp::SimpleNLPModel, x::AbstractVector{T}, j::Integer) where {T}
  @lencheck 2 x
  ...
  if j == 1
    return x[1] - 2 * x[2] + 1
  elseif j == 2
    return -x[1]^2 / nlp.p - x[2]^2 + 1
  end
end

function NLPModels.jth_congrad!(
  nlp::SimpleNLPModel,
  x::AbstractVector{T},
  j::Integer,
  g::AbstractVector{T},
...
function NLPModels.jth_sparse_congrad(nlp::SimpleNLPModel, x::AbstractVector{T}, j::Integer) where {T}
  @lencheck 2 x
  @rangecheck 1 nlp.meta.ncon j
  increment!(nlp, :neval_jgrad)
  if j == 1
  ...
  return sparse([1, 1], [1, 2], vals, 1, nlp.meta.nvar)
end

function NLPModels.varscale(nlp::SimpleNLPModel{T}) where {T}
  return ones(T, nlp.meta.nvar)
end

function NLPModels.lagscale(nlp::SimpleNLPModel{T}) where {T}
  return ones(T, nlp.meta.ncon)
end

function NLPModels.conscale(nlp::SimpleNLPModel{T}) where {T}
  return ones(T, nlp.meta.ncon)
end
