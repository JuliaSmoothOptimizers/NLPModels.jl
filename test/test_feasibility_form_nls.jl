function test_nls_to_cons()
  @testset "Test FeasibilityFormNLS consistency" begin
    ...
    for (F,n,ne) in [(F1,2,2), (F2,5,1), (F3,2,3)],
      ...
      NLPModelsTestUtils.consistent_functions([nlpcon; adnlp], exclude=[ghjvprod])

      adnls = ADNLSModel(x->x[n+1:end], [x0; zeros(ne)], ne,
                        x->[F(x[1:n]) - x[n+1:end]; c(x[1:n])],
                        zeros(ne+m), zeros(ne+m))
      NLPModelsTestUtils.consistent_functions([nlpcon; adnls], exclude=[ghjvprod])
      NLPModelsTestUtils.consistent_nls_functions([nlpcon; adnls])
    end
  end
  ...
  @testset "Test FeasibilityFormNLS with LLSModel" begin
    for n = [10; 30], ne = [10; 20; 30], m = [0; 20]
      for T in [(rows,cols)->Matrix(1.0I, rows, cols) .+ 1,
        ...
        NLPModelsTestUtils.consistent_functions([nlpcon; lls2], exclude=[hess, hess_coord, ghjvprod])
        NLPModelsTestUtils.consistent_nls_functions([nlpcon; lls2])
      end
    end
  end
  ...
  @testset "Test FeasibilityFormNLS of a FeasibilityResidual" begin
    ...
    NLPModelsTestUtils.consistent_functions([ffnls; nlp2])
    NLPModelsTestUtils.consistent_nls_functions([ffnls; nlp2], exclude=[ghjvprod])
    ...
    nlp2 = LLSModel([spzeros(m, n)  I], zeros(m),
                    C=[A -I], lcon=b, ucon=b)
    NLPModelsTestUtils.consistent_functions([ffnls; nlp2], exclude=[hess, hess_coord, ghjvprod])
    NLPModelsTestUtils.consistent_nls_functions([ffnls; nlp2])
  end
